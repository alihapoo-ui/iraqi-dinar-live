...
def format_age_label(last_time: pd.Timestamp) -> str:
    age_minutes = int((pd.Timestamp.now(tz=last_time.tz) - last_time).total_seconds() // 60) if last_time.tzinfo else int((pd.Timestamp.now() - last_time).total_seconds() // 60)
    if age_minutes <= 2:
        return "🟢 Live"
    if age_minutes <= 10:
        return f"🟡 Delayed {age_minutes}m"
    return f"🔴 Stale {age_minutes}m"
...
status_label = format_age_label(latest_ts)
